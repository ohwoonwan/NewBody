...
public class RecordFlyMain extends AppCompatActivity {

    FirebaseFirestore db;
    FirebaseAuth mAuth;

    private boolean flyStartDetected = false;
    private boolean flyEndDetected = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_record_fly_main);
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            if (task.isSuccessful()) {
                                ...
                                if (document.exists()) {
                                    ...
                                    saveFlyScoreWithName(userName, user);
                                    saveFlyRecordWithDate(userName, user);
                                }
                            }
                        }
                    });
                }
                speakFlyResult(score);
                customDialog = new CustomDialog(RecordFlyMain.this
                        ,"시간 : " + (time/60000) + "분 \n기록 : " + score + "개");
                customDialog.show();
            }
        }.start();
    ...
    private void speakFlyResult(int count) {
        String textToSpeak ="총 기록은 " + count + "개 입니다. ";
        tts.speak(textToSpeak, TextToSpeech.QUEUE_FLUSH, null, null);
    }

    private void saveFlyRecordWithDate(String userName, FirebaseUser user){
        Map<String, Object> userData = new HashMap<>();
        final String collectionName = "dailyFlyRecords";

        // 날짜 정보 생성
        DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        String currentDate = dateFormat.format(new Date());

        userData.put(currentDate+"flyCount", score);
        ...
        userRecordRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        // 기존의 스쿼트 수를 가져옵니다.
                        Long existingFlyCount = document.getLong(currentDate+"flyCount");
                        if (existingFlyCount != null) {
                            int newFlyCount = existingFlyCount.intValue() + score;
                            userData.put(currentDate+"flyCount", newFlyCount);
                        }
                    }
                    // 새로운 스쿼트 수를 저장합니다.
    ...
    private void saveFlyScoreWithName(String userName, FirebaseUser user){
        Map<String, Object> userData = new HashMap<>();
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        Long flyCountLong;
                        if (time == 60000) {
                            flyCountLong = document.getLong("countFly1Minute");
                        } else if (time == 120000) {
                            flyCountLong = document.getLong("countFly2Minute");
                        } else {
                            flyCountLong = document.getLong("countFly3Minute");
                        }
                        int existingFlyCount = 0; // 초기 값을 0으로 설정

                        if (flyCountLong != null) {
                            existingFlyCount = flyCountLong.intValue();
                        }

                        if (existingFlyCount <= score) {
                            userData.put(collectionName, score);
...
//                guidelineCanvas.drawRect(0, 0, guidelineBmp.getWidth(), guidelineBmp.getHeight(), transPaint);
    ...
    private void handlePoseDetection(Pose pose) {
        ...
        if (flyStartDetected && isFlyEnd) {
            score++;
            countEx.setText("개수 : " + score);
            speakFlyCount(score);
            flyStartDetected = false; // 다음 연속 감지를 위해 초기화
            flyEndDetected = false;
            checkFly = false;
        } else if (isFlyStart) {
            flyStartDetected = true;
            if(!checkFly){
                speakFly();
                checkFly = true;
            }
    ...
    private void speakFlyCount(int count) {
        String textToSpeak = count + "개";
        tts.speak(textToSpeak, TextToSpeech.QUEUE_FLUSH, null, null);
    }

    private void speakFly() {
    ...
    private void startInit(){
        ...
        cameraProviderFuture.addListener(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    provider.unbindAll();
                    provider.bindToLifecycle(RecordFlyMain.this, CameraSelector.DEFAULT_FRONT_CAMERA, preview);
        ...
        }, ActivityCompat.getMainExecutor(RecordFlyMain.this));
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 2 && resultCode == Activity.RESULT_OK) {
            ...
            if(str.equals("나가기") || str.equals("종료")){
                Intent intent = new Intent(RecordFlyMain.this, Record.class);
