...
public class RecordTricepsMain extends AppCompatActivity {

    FirebaseFirestore db;
    FirebaseAuth mAuth;

    private boolean tricepsStartDetected = false;
    private boolean tricepsEndDetected = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_record_triceps_main);
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            if (task.isSuccessful()) {
                                ...
                                if (document.exists()) {
                                    ...
                                    saveTricepsScoreWithName(userName, user);
                                    saveTricepsRecordWithDate(userName, user);
                                }
                            }
                        }
                    });
                }
                speakTricepsResult(score);
                customDialog = new CustomDialog(RecordTricepsMain.this
                        ,"시간 : " + (time/60000) + "분 \n기록 : " + score + "개");
                customDialog.show();
            }
        }.start();
    ...
    private void speakTricepsResult(int count) {
        String textToSpeak ="총 기록은 " + count + "개 입니다. ";
        tts.speak(textToSpeak, TextToSpeech.QUEUE_FLUSH, null, null);
    }

    private void saveTricepsRecordWithDate(String userName, FirebaseUser user){
        Map<String, Object> userData = new HashMap<>();
        final String collectionName = "dailyTricepsRecords";

        // 날짜 정보 생성
        DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        String currentDate = dateFormat.format(new Date());

        userData.put(currentDate+"tricepsCount", score);
        ...
        userRecordRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        // 기존의 스쿼트 수를 가져옵니다.
                        Long existingTricepsCount = document.getLong(currentDate+"tricepsCount");
                        if (existingTricepsCount != null) {
                            int newTricepsCount = existingTricepsCount.intValue() + score;
                            userData.put(currentDate+"tricepsCount", newTricepsCount);
                        }
                    }
                    // 새로운 스쿼트 수를 저장합니다.
    ...
    private void saveTricepsScoreWithName(String userName, FirebaseUser user){
        Map<String, Object> userData = new HashMap<>();
        ...
        userRecordRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    DocumentSnapshot document = task.getResult();
                    if (document.exists()) {
                        Long tricepsCountLong;
                        if (time == 60000) {
                            tricepsCountLong = document.getLong("countTriceps1Minute");
                        } else if (time == 120000) {
                            tricepsCountLong = document.getLong("countTriceps2Minute");
                        } else {
                            tricepsCountLong = document.getLong("countTriceps3Minute");
                        }
                        int existingTricepsCount = 0; // 초기 값을 0으로 설정

                        if (tricepsCountLong != null) {
                            existingTricepsCount = tricepsCountLong.intValue();
                        }

                        if (existingTricepsCount <= score) {
                            userData.put(collectionName, score);
...
//                guidelineCanvas.drawRect(0, 0, guidelineBmp.getWidth(), guidelineBmp.getHeight(), transPaint);
    ...
    private void handlePoseDetection(Pose pose) {
        ...
        if (tricepsStartDetected && isTricepsEnd) {
            score++;
            countEx.setText("개수 : " + score);
            speakTricepsCount(score);
            tricepsStartDetected = false; // 다음 연속 감지를 위해 초기화
            tricepsEndDetected = false;
            checkTriceps = false;
        } else if (isTricepsStart) {
            tricepsStartDetected = true;
            if(!checkTriceps){
                speakTriceps();
                checkTriceps = true;
            }
    ...
    private void speakTricepsCount(int count) {
        String textToSpeak = count + "개";
        tts.speak(textToSpeak, TextToSpeech.QUEUE_FLUSH, null, null);
    }

    private void speakTriceps() {
    ...
    private void startInit(){
        ...
        cameraProviderFuture.addListener(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    provider.unbindAll();
                    provider.bindToLifecycle(RecordTricepsMain.this, CameraSelector.DEFAULT_FRONT_CAMERA, preview);
        ...
        }, ActivityCompat.getMainExecutor(RecordTricepsMain.this));
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 2 && resultCode == Activity.RESULT_OK) {
            ...
            if(str.equals("나가기") || str.equals("종료")){
                Intent intent = new Intent(RecordTricepsMain.this, Record.class);
