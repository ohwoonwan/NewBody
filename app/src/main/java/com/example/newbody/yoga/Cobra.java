...
public class Cobra extends AppCompatActivity {
    ...
    private TargetPose targetCobraStartSign;
    private TargetPose targetCobraEndSign;
...
//                guidelineCanvas.drawRect(0, 0, guidelineBmp.getWidth(), guidelineBmp.getHeight(), transPaint);
    ...
    private void initTargetPoses() {
        targetCobraStartSign = new TargetPose(
                Arrays.asList(
        ...
        targetCobraEndSign = new TargetPose(
                Arrays.asList(
                        ...
                        new TargetShape(PoseLandmark.RIGHT_HIP, PoseLandmark.RIGHT_SHOULDER, PoseLandmark.RIGHT_ELBOW, 20),
                        new TargetShape(PoseLandmark.LEFT_HIP, PoseLandmark.LEFT_SHOULDER, PoseLandmark.LEFT_ELBOW, 20)
                )
        );
    }
    ...
    private void handlePoseDetection(Pose pose) {
        boolean isCobraStart = isPoseMatching(pose, targetCobraStartSign);
        boolean isCobraEnd = isPoseMatching(pose, targetCobraEndSign);

        if (isCobraEnd) {
            cobraYoga.setText("내려가세요");
            if(!checkDog){
                speakDumbbellStart();
                checkDog = true;
            }
        } else if (isCobraStart && checkDog) {
            cobraYoga.setText("잘했습니다");
            speakDumbbellEnd();
            checkDog = false;
        } else if (!checkDog) {
            cobraYoga.setText("더 올라가세요");
        }
    }
    ...
    private void startInit(){
        ...
        cameraProviderFuture.addListener(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    provider.unbindAll();
                    provider.bindToLifecycle(Cobra.this, CameraSelector.DEFAULT_FRONT_CAMERA, preview);
        ...
        }, ActivityCompat.getMainExecutor(Cobra.this));
    }
